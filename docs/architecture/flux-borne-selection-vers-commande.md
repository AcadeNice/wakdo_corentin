...
sequenceDiagram
    ...
    A->>DB: pay -> transition paid + decrement stock (transaction)
    DB-->>A: { status: "paid", total_ttc_cents }
    A-->>B: 200 { data: {...} }
...
`src/public/borne/assets/js/page-products.js:123`. La couche de donnees
...
(`src/public/borne/assets/js/data.js:65-106`) :
...
- Route : `src/public/admin/index.php:102`
  `GET /api/products -> [CatalogueController::class, 'products']`.
- Controleur : `src/app/Controllers/CatalogueController.php:52-73`. Il appelle
  ...
  (`src/app/Controllers/CatalogueController.php:225-258`).
- Requete SQL :
  `src/app/Catalogue/ProductRepository.php:112-127`
...
categorie (`src/public/borne/assets/js/data.js:79-96`). La salade arrive dans le
...
`CATEGORY_ID_TO_SLUG` (`src/public/borne/assets/js/data.js:217-227` ;
`salades` = id 7), puis rend les tuiles
(`src/public/borne/assets/js/page-products.js:37-112`).

Au tap sur une tuile, le handler de clic decide du comportement
(`src/public/borne/assets/js/page-products.js:104-109`) :
...
> `src/app/Order/OrderRepository.php:729-763`), mais la modale produit de la borne
...
apres chaque mutation (`src/public/borne/assets/js/order-panel.js:155-186`). Le
vue-modele pur `buildPanelModel()`
(`src/public/borne/assets/js/order-panel.js:74-85`) calcule le total cote client
...
(`src/public/borne/assets/js/order-panel.js:31-35`). Ce total client sert
...
(`src/public/borne/assets/js/checkout.js:144-165`).
...
- `idempotency_key` : cle STABLE par tentative, memorisee en `sessionStorage`
  (`src/public/borne/assets/js/checkout.js:109-120`). Un retry reseau reutilise la
  meme cle.

### 4.b Les deux appels

`submitOrder()` (`src/public/borne/assets/js/checkout.js:144-165`) :

```js
const created = await postJson('/api/orders', payload);       // 1er appel
...
1. `POST /api/orders` cree la commande en `pending_payment` et renvoie son
   `order_number`.
2. `POST /api/orders/{order_number}/pay` transitionne la commande vers `paid`.

Le declenchement vient de `page-payment.js` : le clic sur un mode de paiement
appelle `startCheckout()` puis `doSubmit()`
(`src/public/borne/assets/js/page-payment.js:45-78,196-197`). En sur-place, la
...
> creee et encaissee cote serveur (`src/public/borne/payment.html:50-53`).
...
`src/public/admin/index.php:91-92`
...
> (`src/app/Order/OrderRepository.php:101-112`).
...
`orderError()` (`src/app/Controllers/OrderController.php:112-125`) :
...
if ($existing !== null) {
    return $existing;          // anti double-charge : on renvoie la commande deja creee
}
...
(`src/app/Order/OrderRepository.php:41-60`). La colonne porte une contrainte
...
`persist()` re-resout chaque ligne et RE-CALCULE les prix depuis la base
(`src/app/Order/OrderRepository.php:169-274`). Le client n'envoie que
`product_id` + `quantity` ; aucun prix client n'est lu dans le code.

- Validation du `service_mode` et du `service_tag`
  (`src/app/Order/OrderRepository.php:173-180`).
- Garde rupture de stock RG-T21 (set calcule une fois)
  (`src/app/Order/OrderRepository.php:191`).
- Resolution ligne par ligne via `resolveLine()`
  (`src/app/Order/OrderRepository.php:194,609-656`). Pour un produit a la carte
  (`src/app/Order/OrderRepository.php:615-630`) :
...
(`src/app/Order/OrderRepository.php:770-787`) :

```php
$unitHt = (int) round($unitTtc * 1000 / (1000 + $vat));
```

Les totaux commande sont la somme des lignes
(`src/app/Order/OrderRepository.php:196-205`).

**INSERT dans une transaction unique** (`src/app/Order/OrderRepository.php:209-271`).

`customer_order` ecrit (`src/app/Order/OrderRepository.php:210-225`) :
...
**Generation du numero `order_number`** (`src/app/Order/OrderRepository.php:226-231`) :
...
> `src/app/Order/OrderRepository.php:18-20`).

**Snapshots** ecrits sur `order_item` (`src/app/Order/OrderRepository.php:233-250`) :
...
`db/migrations/0001_init_schema.sql:352-354`. Pour un menu, `persist()` ecrit
aussi `order_item_selection` et `order_item_modifier`
(`src/app/Order/OrderRepository.php:253-266`) ; pour un produit a la carte sans
modificateur, ces deux tables ne recoivent rien.

### 6.b Encaissement : `pay`

`pay()` (`src/app/Order/OrderRepository.php:305-363`) lit la commande par numero,
gere l'idempotence de transition, puis fait la transition + le decrement de stock
dans UNE transaction.

Idempotence / transitions (`src/app/Order/OrderRepository.php:322-328`) :

```php
if ($status === 'paid')             return $result;        // deja paye -> pas de re-decrement
if ($status !== 'pending_payment')  throw INVALID_TRANSITION; // delivered / cancelled
```

Transition atomique gardee + decrement de stock RG-T20
(`src/app/Order/OrderRepository.php:331-360`) :
...
- La garde `status = 'pending_payment'` dans le `WHERE` du `UPDATE` assure qu'en
  ...
  sort idempotent).
- `consumption()` (`src/app/Order/OrderRepository.php:542-600`) agrege les unites
...
- Chaque mouvement est trace dans `stock_movement` (`movement_type='sale'`,
  ...
  `db/migrations/0001_init_schema.sql:423`.

> Nuance documentee dans le code (`src/app/Order/OrderRepository.php:296-298`) :
> le decrement est inerte tant qu'un produit n'a pas de recette
> (`product_ingredient`). La transition `paid` s'applique de toute facon ; le
...
`POST /api/orders/{number}/pay` renvoie `200` avec `status: "paid"`.

Cote borne, `submitOrder()` retient `order_number` et `total_ttc_cents`
(`src/public/borne/assets/js/checkout.js:158-164`). `page-payment.js` memorise le
...
montant regle (`src/public/borne/assets/js/page-confirmation.js:21-39`), injectes
dans `#order-number` et `#order-total` de `confirmation.html`
(`src/public/borne/confirmation.html:45,49`). En l'absence de commande soumise
(visite directe), un numero de repli local `WK-<timestamp>` est genere
(`src/public/borne/assets/js/page-confirmation.js:17-19,34`).
...
1. **Separation front / back nette.** Le panier vit entierement cote client
   ...
   paiement (`src/public/borne/assets/js/checkout.js:157-161`). Le total affiche
...
4. **Idempotence (anti double-charge).** `idempotency_key` genere cote borne et
   stable par tentative (`src/public/borne/assets/js/checkout.js:109-120`),
   verifie a la creation (`src/app/Order/OrderRepository.php:103-107`) avec
   ...
   (`src/app/Order/OrderRepository.php:322-324`).

5. **Decrement de stock atomique (RG-T20).** Transition `paid` + decrement +
   `stock_movement` dans une seule transaction
   (`src/app/Order/OrderRepository.php:331-360`). La garde
   `WHERE ... status = 'pending_payment'` et l'ordre de verrou stable par
   `ingredient_id` (`ksort`, `src/app/Order/OrderRepository.php:597`) protegent
...
6. **Conventions HTTP.** Enveloppe standard `{ data }` (succes) /
   ...
   (`src/app/Controllers/OrderController.php:43,57,112-125`). Le flux en deux POST
...
- **Flux a deux appels** : confirme dans le code
  (`src/public/borne/assets/js/checkout.js:157,161`) — POST creation puis POST
  pay, et non un POST unique.
- **Modificateurs d'ingredient** : le contrat serveur les supporte
  (`src/app/Order/OrderRepository.php:729-763`) mais la modale produit borne lue
