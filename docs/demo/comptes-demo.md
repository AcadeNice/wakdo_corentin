...
| Équipier cuisine | `cuisine@wakdo.local` | `WakdoCuisine2026!` | `2020` | kitchen | Lire le catalogue (produits/menus) ; lire le stock et faire l'inventaire ; lire les commandes (`order.read`) — ce qui donne accès à l'écran cuisine `/kitchen/display` (file filtrée sur les 3 canaux, kitchen voit tout) et fait avancer une commande payée à « prête » ; ainsi qu'à la liste brute `/admin/orders` (voir la note de vérification ci-dessous). | Créer une commande, la livrer (`order.deliver`) ou l'annuler (`order.cancel`) ; modifier le catalogue ou le stock ; consulter les statistiques ; gérer comptes/rôles. |
| Équipier comptoir (1er équipier) | `comptoir@wakdo.local` | `WakdoComptoir2026!` | `3030` | counter | Lire le catalogue ; lire le stock et faire l'inventaire ; créer, livrer et annuler une commande comptoir (`/counter/orders`) ; lire la liste globale `/admin/orders` ; écran cuisine filtré aux sources `kiosk`+`counter`. | Modifier le catalogue (produits/menus/ingrédients) ; réapprovisionner le stock (`stock.manage`) ; consulter les statistiques ; gérer comptes/rôles. |
...
bloc « Pilotage »). La différence entre les deux pages : `/admin/orders` liste
toutes les commandes, tous canaux confondus (`OrderQueryRepository::recent()`,
non filtrée par source), alors que `/kitchen/display` filtre par
`role_visible_source` (`OrderQueryRepository::paidQueue()`/`visibleSources()`).
Le filtrage par canal (RG-T12) s'applique donc à l'écran cuisine, pas à la
liste admin des commandes — une nuance vérifiée dans le code, pas supposée.
